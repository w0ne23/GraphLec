...
def group_segments_by_slide_and_context_iterative(
    segments: list[dict],
    slide_ranges: list[dict],
    duration_sec: float,
) -> list[dict]:
    """
    슬라이드별로 세그먼트를 나누고, 각 슬라이드 내부에서
    이전까지 병합된 컨텍스트와 다음 세그먼트를 LLM으로 비교하면서
    순차적으로 맥락을 병합한 컨텍스트 구조를 생성.

    반환: slides_structure_iter
      [ { slide_index, start_sec, end_sec, text, contexts: [ { context_index, start, end, text, segment_indices, segments: [...] } ] } ]
    """
    if not segments or not slide_ranges:
        return []

    # 세그먼트 -> 슬라이드 매핑은 기존 함수와 동일
    seg_to_slide: list[int | None] = []
    for i, seg in enumerate(segments):
        t = seg["start"]
        slide_idx = None
        for r in slide_ranges:
    ...
    for r in slide_ranges:
        ...
        # iterative 병합: current 덩어리와 다음 세그먼트를 계속 비교
        contexts: list[dict] = []
        current = initial_groups[0]
        for g_next in initial_groups[1:]:
            if decide_merge_pair(current.get("text", ""), g_next.get("text", "")):
                current = {
                    "start": current["start"],
                    "end": g_next["end"],
                    "text": f"{current['text']} {g_next['text']}".strip(),
                    "segment_indices": list(current["segment_indices"]) + list(g_next["segment_indices"]),
                }
            else:
        ...
        slide_text = " ".join((g["text"] for g in contexts if g["text"])).strip()
        contexts_for_slide: list[dict] = []
        for cix, g in enumerate(contexts):
            segs_in_context = [
                {
