...
- TRANSCRIBE_CHUNK_MAX_SEC        (default: 600)  청크 최대 길이
...
CHUNK_MAX_SEC       = float(os.getenv("TRANSCRIBE_CHUNK_MAX_SEC",     "600"))
...
def build_speech_chunks(
    ...
    """
    무음 구간 정보를 바탕으로 발화(speech) 청크를 산출한다.

    동작:
    - split_min 이상 무음만 청크 경계로 사용 (그 무음은 API에 보내지 않음 = 드롭)
    - split_min 미만 무음은 무시 (청크 안에 그대로 포함)
    - 청크가 chunk_max를 넘으면 강제로 분할 (가장 가까운 무음 우선, 없으면 길이로)
    - 청크가 chunk_min 미만이면 다음 청크와 묶음 (Whisper 컨텍스트 보존)
    ...
    # 4) chunk_min 미만 segment를 다음 segment와 묶음
    merged: list[tuple[float, float]] = []
    i = 0
    while i < len(padded):
        s, e = padded[i]
        # 길이가 너무 짧으면 다음 segment와 합침 (단, 그 사이 무음이 split_min 이상이라 끊겼다면 합치지 않음 — 이미 분리된 이유가 있음)
        # 여기서는 단순화: padding 후 인접 segment 끝과 다음 시작이 가까우면(작은 gap) 합침
        while (e - s) < chunk_min and (i + 1) < len(padded):
            ns, ne = padded[i + 1]
            # 두 segment 사이 gap이 너무 크면(즉 큰 무음) 합치지 않음
            gap = ns - e
            if gap > split_min:
                break
            # 합치되 chunk_max 초과 방지
    ...
    # 5) chunk_max 초과 segment는 강제 분할 (단순 길이 분할)
...
def _transcribe_chunks_from_audio(
    ...
    """
    이미 추출된 audio_path를 chunks 단위로 잘라 Groq에 전송한다.
    ...
    for i, (cs, ce) in enumerate(chunks):
        ...
        if dur <= 0:
            continue
        chunk_path = str(base_dir / f"temp_chunk_{label_prefix}{i}.wav")
        cmd = [
            "ffmpeg", "-ss", f"{cs}", "-t", f"{dur}",
        ...
        if transcription is not None:
            for seg in transcription.segments:
                ...
                # → audio_path 내 절대 시간 = chunk_start + seg_start
                # → 영상 절대 시간     = time_offset + audio 내 절대 시간
                abs_start = time_offset + cs + float(seg["start"])
                abs_end   = time_offset + cs + float(seg["end"])
                words = _normalize_words(seg.get("words"), chunk_start=time_offset + cs)
                segments.append({
                    "start": abs_start,
                    "end":   abs_end,
