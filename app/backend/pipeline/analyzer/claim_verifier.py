...
이 단계는 crosscheck 전의 1차 후보 추출 단계입니다. 최종 확정, 교수 피드백 작성, 정밀한 issue type 확정은 후속 단계에서 수행합니다.
...
3. type, issue_type, utterance_id는 출력하지 마세요.
...
def _judge_issue_candidates(
    ...
    full_prompt = _build_issue_candidate_prompt(claims, utterances, current_date, hint, slide_ctx)
...
def judge_issue_candidates_only(
    ...
    """crosscheck 이전 1차 judge만 실행해 issue 후보를 반환한다."""
    from . import claim_common as cv
