...
def _build_grounding_prompt(issue: dict, hint: dict) -> str:
...
판단 기준:
1. 검색 결과에서 지적 내용을 뒷받침하는 근거를 찾으세요.
2. 검색 결과가 오히려 발화 원문이 맞다고 지지하면, 이 지적은 기각합니다.
3. 검색 결과가 불충분하거나 모호하면, 이 지적은 보류(기각)합니다.
...
def _ground_verify_issue(issue: dict, hint: dict) -> tuple[dict, dict]:
    from . import claim_common as cv

    prompt = _build_grounding_prompt(issue, hint)
...
def ground_verify_all_issues(
    ...
    def process(i, issue):
        ...
        return _ground_verify_issue(issue.copy(), hint)
