...
강한 표현을 문자 그대로의 독점 수행, 모든 내부 동작의 대행, 또는 모든 예외의 배제로 바로 해석하지 마세요.
제공 문맥상 일반적인 역할 설명이나 표준 접근 경로 설명으로 자연스럽게 성립하면 is_valid_issue는 0.39 이하로 주세요.
문제를 만들기 위해 "관리한다/권한이 있다/요청한다/대신한다"를 "해당 주체만 모든 실제 동작을 직접 수행한다"로 바꿔 읽어야 한다면 is_valid_issue는 0.29 이하로 주세요.

중간 계층/중재자/관리자/권한 주체가 있는 시스템 설명에서 "직접 못 한다", "직접 하지 않는다", "대신 해준다"는 표현은
대개 "중재자나 서비스를 우회해서는 못 한다"는 뜻일 수 있습니다.
이 경우를 "해당 주체가 논리적 작업에 전혀 참여하지 못한다"는 뜻으로 확대하지 마세요.
현재 anchor가 "직접 못 한다"류의 우회 금지/중재 필요 설명이고,
문맥이 중재자에게 요청하거나 권한을 받아 수행하는 흐름을 설명한다면 일반적인 강의 설명으로 보고 is_valid_issue는 0.29 이하로 주세요.

"결국", "궁극적으로", "최종적으로", "간접적으로"처럼 중간 과정을 압축하는 표현이 있으면
그 설명을 직접 호출/직접 동일시로 해석하지 마세요.
중간 계층을 생략했더라도 강의 맥락에서 최종 책임 주체나 최종 자원 제공 주체를 설명하는 말로 자연스럽게 읽히면 is_valid_issue는 0.29 이하로 주세요.
중간 계층 생략만으로 D형을 유지하려면, 학생이 실제 호출 주체/과정/조건을 잘못 예측하게 되는 구체 오개념이 문맥에 남아야 합니다.
"결국 A에게 요청한다"는 표현은 "항상 즉시 A를 직접 호출한다"가 아닙니다.
입문 설명에서 중간 계층을 생략하고 최종 책임 주체를 말한 정도라면 not_issue로 판단하세요.
...
보이는 것/관찰/접근 가능성을 설명하는 문맥에서는
"잘 볼 수 없다", "보이지 않는다", "직접 보기 어렵다" 같은 표현을
어떤 방법으로도 검사하거나 확인할 수 없다는 절대 불가능 명제로 바꿔 읽지 마세요.
문맥상 사용자에게 직접 노출되는 화면/UI/도구와 내부 구성요소를 대비하는 설명이면
일반적인 가시성/접근성 설명으로 보고 is_valid_issue는 0.29 이하로 주세요.
이 경우 문제를 만들려면 "메모리에 있으면 원칙적으로 볼 수 없다"처럼 원문보다 강한 인과를 만들어야 하므로 낮게 보세요.

접근 권한/통제/중재를 설명하는 문맥에서 "독점 사용", "독점 권한", "배타적 권한" 같은 표현은
실제 자원 소비 주체가 하나라는 뜻보다 접근 통제 권한이 중앙에 있다는 뜻일 수 있습니다.
문맥이 요청, 접근 권한, 허용, 할당, 공유 같은 통제 흐름을 함께 설명하면 실제 사용 주체 오개념으로 확대하지 말고 is_valid_issue는 0.29 이하로 주세요.
이때 "사용"이라는 단어 하나를 "물리적/실질적 소비 주체"로 바꿔 읽어야만 문제가 된다면 D형 issue가 아닙니다.

원인 설명이 거칠더라도, 뒤 문맥이 대비/예시/정의로 실제 의미를 좁히고 결론 자체가 일반 도메인 지식 기준으로 맞으면 낮게 보세요.
"A라서 B다"가 엄밀한 원인 설명은 아니더라도, 강의 흐름상 "A는 B처럼 드러나는 대상이 아니다" 정도의 대비 설명이면 is_valid_issue는 0.39 이하로 주세요.
...
- 현재 anchor가 중재자/권한/서비스를 우회할 수 없다는 뜻으로 자연스럽게 읽히면 is_valid_issue는 0.29 이하로 주세요.
- "결국/궁극적으로/최종적으로"가 붙은 간접 경로 설명을 직접 동일시로 바꿔야만 문제가 되면 is_valid_issue는 0.29 이하로 주세요.
...
- 사용자에게 보이는 UI/도구와 내부 구성요소를 대비하는 맥락의 "잘 볼 수 없다"류 표현을 절대적 관찰 불가능으로 바꿔야만 문제가 되면 is_valid_issue는 0.29 이하로 주세요.
- 접근 권한/통제/중재 문맥의 독점 표현을 실제 자원 소비 주체 오류로 바꿔 읽어야만 문제가 되면 is_valid_issue는 0.29 이하로 주세요.
