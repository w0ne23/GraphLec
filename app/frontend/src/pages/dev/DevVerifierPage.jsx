...
import '../../styles/dev-verifier.css'
...
function SummaryMetric({ label, value, tone = '', active = false, onClick }) {
  return (
    <button
      className={`dev-vf-metric ${tone ? `dev-vf-metric--${tone}` : ''} ${active ? 'dev-vf-metric--active' : ''}`}
      onClick={onClick}
    >
      <div className="dev-vf-metric-value">{value}</div>
      <div className="dev-vf-metric-label">{label}</div>
    </button>
  )
}
...
function IssueTypeBreakdown({ items, activeFilter = 'all', onFilterChange }) {
  ...
  return (
    <div className="dev-vf-type-breakdown">
      {ISSUE_FILTERS.map((filter) => {
        const count = counts[filter.key] || 0
        return (
          <button
            className={`dev-vf-type-pill ${activeFilter === filter.key ? 'dev-vf-type-pill--active' : ''} ${count ? '' : 'dev-vf-type-pill--empty'}`}
            key={filter.key}
...
function IssueFilterDescription({ filter }) {
  ...
  return (
    <div className="dev-vf-filter-description">
      <strong>{label}</strong>
...
function DetailRow({ label, value }) {
  ...
  return (
    <div className="dev-vf-detail-row">
      <dt>{label}</dt>
      <dd>{value}</dd>
    </div>
  )
...
function EvidenceSources({ sources }) {
  ...
  return (
    <div className="dev-vf-evidence-block">
      <div className="dev-vf-evidence-title">근거 링크</div>
      <div className="dev-vf-source-list">
...
function ClaimCard({ claim, section, expanded, onToggle, onWatch }) {
  ...
  return (
    <article className={`dev-vf-claim-card ${expanded ? 'dev-vf-claim-card--expanded' : ''}`}>
      <button className="dev-vf-claim-main" onClick={onToggle}>
        <div className="dev-vf-claim-copy">
          <div className="dev-vf-claim-title">{title}</div>
          <div className="dev-vf-chip-row">
            <span className="dev-vf-chip dev-vf-chip--stage">{labelForStage(claim.stage || section)}</span>
            {displayIssueKey && <span className={`dev-vf-chip dev-vf-chip--${displayIssueKey}`}>{displayIssueLabel}</span>}
            {hasSeverityScore && (
              <span className={`dev-vf-chip dev-vf-chip--score dev-vf-chip--score-${severityStatus || 'unknown'}`}>
                점수 {scoreLabel(claim.severity_score)}
              </span>
            )}
          </div>
        </div>
        <div className="dev-vf-claim-meta">
      ...
      {expanded && (
        <div className="dev-vf-claim-detail">
          <dl>
      ...
      {canWatch && (
        <div className="dev-vf-claim-actions">
          <button className="dev-vf-watch-btn" onClick={onWatch}>영상 보기</button>
        </div>
      )}
    </article>
  )
...
function SlideErrorItem({ error }) {
  ...
  return (
    <div className="dev-vf-error-item">
      <div className="dev-vf-error-main">
        <div className="dev-vf-error-fields">
          <div className="dev-vf-error-correction">
            <span>수정</span>
            <strong>
          ...
          {error.error_type_label && (
            <div className="dev-vf-error-field">
              <span>오류 유형</span>
        ...
        <div className="dev-vf-error-meta">
      ...
      {candidates.length > 1 && (
        <div className="dev-vf-candidate-list">
          <b>후보 수정안</b>
...
function SlideErrorCard({ group, review = false }) {
  return (
    <article className={`dev-vf-error-slide-card ${review ? 'dev-vf-error-slide-card--review' : ''}`}>
      <div className={`dev-vf-error-slide-layout ${group.imageUrl ? '' : 'dev-vf-error-slide-layout--no-image'}`}>
        {group.imageUrl && (
          <div className="dev-vf-error-slide-image">
            <img src={group.imageUrl} alt={`Slide ${group.slideNumber}`} loading="lazy" />
          </div>
        )}
        <div className="dev-vf-error-slide-panel">
          <div className="dev-vf-error-slide-head">
            <strong>slide {group.slideNumber}</strong>
            <span>{group.items.length}건</span>
          </div>
          <div className="dev-vf-error-items">
...
export default function DevVerifierPage() {
  const { id } = useParams()
  ...
  function renderClaimList(items, section) {
    return (
      <div className="dev-vf-claim-list">
        {items.map((claim, idx) => {
  ...
  function renderSlideErrorGroups(items, review = false) {
    return (
      <div className="dev-vf-error-list">
      ...
      return (
        ...
        >
          ...
          {filteredReview.length > 0
            ...
            : <div className="dev-vf-empty">선택한 유형의 강의자 확인 이슈가 없습니다.</div>}
        </Section>
      )
    }
    ...
    return (
      ...
      >
        ...
        {filteredFinal.length > 0
          ...
          : <div className="dev-vf-empty">선택한 유형의 확정 이슈가 없습니다.</div>}
      </Section>
    )
  }
  ...
  return (
    <div className="dev-vf-shell">
      <div className="dev-vf-topbar">
        <div className="dev-vf-topbar-left">
          <button className="lp-back" onClick={() => navigate(-1)}>← 이전으로</button>
          <span className="lp-title">{lecture.title} · Verifier</span>
        </div>
        {isVideoMode && (
          <button className="dev-vf-exit-video-btn" onClick={() => setIsVideoMode(false)}>
            영상 닫기
          </button>
        )}
      </div>

      <div className={`dev-vf-body ${isVideoMode ? 'dev-vf-body--video' : ''}`}>
        {isVideoMode && (
          <section className="dev-vf-video-pane">
            <VideoPlayer
              lecture={lecture}
              scenes={[]}
              currentScene={0}
        ...
        <section className="dev-vf-list-pane">
          <div className="dev-vf-summary-card">
            <SummaryMetric
              label="확정 이슈"
              value={finalCount}
              tone="danger"
