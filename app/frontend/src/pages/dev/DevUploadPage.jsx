    ...
    <div className="up-page">
      <div className="up-content">
        <header className="up-header">
          <h1 className="up-title">Upload</h1>
          <p className="up-subtitle">강의 영상을 업로드하면 자동으로 분석합니다.</p>
        </header>
        ...
        <div className="up-panel">
          ...
          <section className="up-form-section">
            <div className="up-form-inner">
              <div className="up-section-title">업로드</div>
              ...
              >
                ...
                <input ref={fileRef} className="up-file-input" type="file" accept="video/*" onChange={e => setFileWithAutoTitle(e.target.files[0])} />
              </div>
              ...
              <button className="up-submit-btn" onClick={handleSubmit} disabled={!file}>업로드 시작</button>
            </div>
          </section>

          {/* ── 섹션 2: 강의 목록 ── */}
          <section className="up-list-section">
            <div className="up-list-header">
              <div className="up-section-title">강의 목록</div>
                  ...
                  <div key={lec.id}>
                    <div className={`upload-row${lec.status === 'done' ? ' upload-row--done' : ''}`} onClick={() => lec.status === 'done' && navigate(`/lectures/${lec.id}`)}>
                      ...
                      <div className="upload-row-actions">
                        {lec.status === 'done' && <button className="upload-btn-verifier" onClick={(e) => { e.stopPropagation(); navigate(`/dev/lectures/${lec.id}/verifier`) }}>Verifier</button>}
                    ...
                    {(lec.status === 'running' || lec.status === 'pending' || lec.status === 'error') && lec.pipeline_stages && (
                      <div className="upload-pipe upload-pipe--attached">
                        ...
                        {lec.status === 'error' && <div className="upload-pipe-error">오류: {lec.error_message}</div>}
                      </div>
                    )}
                  </div>
                )
