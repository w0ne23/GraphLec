...
function idNumberedLabel(label, id, prefix, fallback) {
  return `${label} ${idNumber(id, prefix) || fallback}`
}

function sceneIdFromIndex(value) {
...
function resultFileUrl(value, resultId) {
...
function statusFromSeverityScore(score) {
...
function makeFilter(type, value, label, tab) {
  return { type, value, label, tab }
}
...
function claimRowDomId(claimId) {
...
function sceneValueFromId(id, fallbackIndex) {
  return idValueFromId(id, 'SC', fallbackIndex + 1)
}

function contextValueFromId(id, fallbackIndex) {
  return idValueFromId(id, 'C', fallbackIndex + 1)
}

function claimValueFromId(id, fallbackIndex) {
...
function EmptyFiltered({ activeFilter }) {
  return (
...
function InlineBlock({ label, children }) {
...
function ModelEvidenceSection({ items, valueFormat, title = '모델별 판단' }) {
...
function FinalScoreSummary({ severity }) {
...
function RatioBar({ value, total }) {
  const number = Number(value)
  const base = Number(total)
  if (!Number.isFinite(number) || !Number.isFinite(base) || base <= 0) return null
  const width = Math.max(0, Math.min(1, number / base)) * 100
  return (
    <div className="vf-ratio-bar" aria-hidden="true">
      <i style={{ width: `${width}%` }} />
    </div>
  )
}

function OverviewAction({ label, value, hint, filter, activeFilter, onSelectFilter, disabled = false, tone = '' }) {
  const isActive = isSameFilter(activeFilter, filter)
  const className = [
    'vf-overview-action',
    tone ? `vf-overview-action--${tone}` : '',
    isActive ? 'vf-overview-action--active' : '',
  ].filter(Boolean).join(' ')
  ...
  return (
    <button
      ...
      onClick={() => filter && onSelectFilter(filter)}
    >
      <span>{label}</span>
      <span className="vf-bold">{value}</span>
      {hint && <span className="vf-overview-action-hint">{hint}</span>}
    </button>
  )
}
...
function CompressionMap({ model, statuses, slideStatus, activeTab, onSelectTab }) {
  ...
  const slideReady = hasDataForTab(model, 'slide_review')
  const claimCount = claimReady ? model.claims.length : null
  const resolvedCount = claimReady ? model.claims.filter(claim => claim.resolution_status === 'resolved').length : null
  const needsContextCount = claimReady ? model.claims.filter(claim => claim.needs_context).length : null
  const issueCount = issueReady ? model.issueCandidates.length : null
  const classifiedCount = typeReady ? model.issueTypeRows.length : null
  const reviewNeeded = finalReady ? finalReviewNeededCount(model) : null
  const flowSummary = issueReady
    ? `${claimCount}개 클레임 / ${issueCount}개 이슈 후보`
    : claimReady
      ? `${claimCount}개 클레임`
      : statusText(statuses[0])

  return (
    <div className="vf-compression-map">
      <div className="vf-compression-head">
        <span className="vf-bold">흐름 보고서</span>
        <span>{flowSummary}</span>
      </div>
      <div className="vf-compression-flow">
        <button
          className={`vf-flow-block vf-flow-block--${statuses[0]} ${activeTab === 'claim_extraction' ? 'vf-flow-block--active' : ''}`}
          type="button"
          disabled={!claimReady}
          onClick={() => onSelectTab('claim_extraction')}
        >
          <span>1 주장 추출</span>
          <span className="vf-bold">{claimReady ? claimCount : '-'}</span>
          <span className="vf-flow-block-sub">{claimReady ? `resolved ${resolvedCount} / needs_context ${needsContextCount}` : statusText(statuses[0])}</span>
        </button>
        <div className="vf-flow-arrow" />
        <button
          className={`vf-flow-block vf-flow-block--${statuses[1]} ${activeTab === 'issue_judge' ? 'vf-flow-block--active' : ''}`}
          type="button"
          disabled={!issueReady}
          onClick={() => onSelectTab('issue_judge')}
        >
          <span>2 이슈 후보 판단</span>
          <span className="vf-bold">{issueReady ? issueCount : '-'}</span>
          <span className="vf-flow-block-sub">{issueReady ? `no_issue ${model.issueJudgeSummary.no_issue_claim_count ?? 0}개` : statusText(statuses[1])}</span>
        </button>
        <div className="vf-flow-arrow" />
        <button
          className={`vf-flow-block vf-flow-block--${statuses[2]} ${activeTab === 'issue_classification' ? 'vf-flow-block--active' : ''}`}
          type="button"
          disabled={!typeReady}
          onClick={() => onSelectTab('issue_classification')}
        >
          <span>3 유형 분류</span>
          <span className="vf-bold">{typeReady ? classifiedCount : '-'}</span>
          <span className="vf-flow-block-sub">{typeReady ? `${Object.keys(model.issuesByType).length}개 유형` : statusText(statuses[2])}</span>
        </button>
        <div className="vf-flow-arrow" />
        <button
          className={`vf-flow-block vf-flow-block--${statuses[3]} ${activeTab === 'final_verification' ? 'vf-flow-block--active' : ''}`}
          type="button"
          disabled={!finalReady}
          onClick={() => onSelectTab('final_verification')}
        >
          <span>4 최종 평가</span>
          <span className="vf-bold">{finalReady ? reviewNeeded : '-'}</span>
          <span className="vf-flow-block-sub">{finalReady ? '검토 필요' : statusText(statuses[3])}</span>
        </button>
        <span className="vf-flow-divider" aria-hidden="true" />
        <button
          className={`vf-flow-block vf-flow-block--slide vf-flow-block--${slideStatus} ${activeTab === 'slide_review' ? 'vf-flow-block--active' : ''}`}
          type="button"
          disabled={!slideReady}
          onClick={() => onSelectTab('slide_review')}
        >
          <span>5 슬라이드 오류</span>
          <span className="vf-bold">{slideReady ? model.slideFindings.length : '-'}</span>
          <span className="vf-flow-block-sub">{slideReady ? `보고 대상 ${model.slideErrors.summary?.reportable_error_count ?? '-'}개` : statusText(slideStatus)}</span>
        </button>
      </div>
    </div>
  )
...
function ModelUncertaintyPanel({ model, statuses, activeFilter, onSelectFilter }) {
  const issueDone = isStageDone(statuses[1])
  const summary = model.issueJudgeSummary
  const issueCounts = asObject(summary.issue_counts_by_model)
  const exclusive = asObject(model.issueJudgeCompare.exclusive_by_model)
  const maxIssueCount = Math.max(...Object.values(issueCounts).map(Number).filter(Number.isFinite), 1)

  return (
    <div className="vf-weight-panel vf-weight-panel--model">
      <div className="vf-weight-panel-head">
        <span className="vf-bold">모델 판정 불일치</span>
        <span>{issueDone ? `single_model_only ${summary.single_model_only_count ?? 0}개` : statusText(statuses[1])}</span>
      </div>
      <div className="vf-model-actions">
        <OverviewAction
          label="all_models_agreed"
          value={issueDone ? summary.all_models_agreed_count ?? '-' : '-'}
          hint="강한 후보"
          filter={makeFilter('agreement', 'all_models_agreed', 'all_models_agreed', 'issue_judge')}
          activeFilter={activeFilter}
          onSelectFilter={onSelectFilter}
          disabled={!issueDone}
          tone="agreement"
        />
        {Object.entries(exclusive).map(([modelName, claimIds]) => (
          <OverviewAction
            key={modelName}
            label={`${modelName} 단독`}
            value={issueDone ? asArray(claimIds).length : '-'}
            hint="단독 후보"
            filter={makeFilter('model_only', modelName, `${modelName} 단독 후보`, 'issue_judge')}
            activeFilter={activeFilter}
            onSelectFilter={onSelectFilter}
            disabled={!issueDone}
          />
      ...
      <div className="vf-model-bars">
        {Object.entries(issueCounts).map(([modelName, count]) => (
          <div key={modelName}>
            <span>{modelName}</span>
            <span className="vf-bold">{count}</span>
            <RatioBar value={count} total={maxIssueCount} />
          </div>
        ))}
        {!issueDone && <div className="vf-overview-empty">모델 판정 대기</div>}
      </div>
    </div>
  )
}

function PriorityPanel({ model, statuses, activeFilter, onSelectFilter }) {
  const finalDone = isStageDone(statuses[3])
  const finalSummary = asObject(model.issueVerifier.summary)
  const highSeverity = finalDone ? finalSummary.high_severity_count ?? 0 : '-'
  const manualReview = finalDone ? finalSummary.needs_manual_review_count ?? 0 : '-'

  return (
    <div className="vf-weight-panel vf-weight-panel--priority">
      <div className="vf-weight-panel-head">
        <span className="vf-bold">최종 우선순위</span>
        <span>{finalDone ? `${model.severityItems.length}개 이슈` : statusText(statuses[3])}</span>
      </div>
      <div className="vf-priority-actions">
        <OverviewAction
          label="고심각"
          value={highSeverity}
          hint="먼저 검토"
          filter={makeFilter('high_severity', 'high', '고심각 이슈', 'final_verification')}
          activeFilter={activeFilter}
          onSelectFilter={onSelectFilter}
          disabled={!finalDone}
          tone="danger"
        ...
        <OverviewAction
          ...
          value={manualReview}
          hint="판단 보류"
          filter={makeFilter('manual_review', 'needs', '수동 검토 필요', 'final_verification')}
          activeFilter={activeFilter}
          onSelectFilter={onSelectFilter}
          disabled={!finalDone}
          tone="warning"
      ...
      <div className="vf-final-status-actions">
        <OverviewAction
          label="확정"
          value={finalDone ? model.counts.confirmed : '-'}
          filter={makeFilter('final_status', 'confirmed', '확정 이슈', 'final_verification')}
          activeFilter={activeFilter}
          onSelectFilter={onSelectFilter}
          disabled={!finalDone}
        />
        <OverviewAction
          label="검토"
          value={finalDone ? model.counts.review : '-'}
          filter={makeFilter('final_status', 'professor_check', '검토 필요', 'final_verification')}
          activeFilter={activeFilter}
          onSelectFilter={onSelectFilter}
          disabled={!finalDone}
        />
        <OverviewAction
          label="기각"
          value={finalDone ? model.counts.rejected : '-'}
          filter={makeFilter('final_status', 'rejected', '기각 이슈', 'final_verification')}
          activeFilter={activeFilter}
          onSelectFilter={onSelectFilter}
          disabled={!finalDone}
        />
...
function TypeDistributionStrip({ model, statuses, activeFilter, onSelectFilter }) {
  const typeDone = isStageDone(statuses[2])
  const rows = Object.entries(model.issuesByType)
  const total = model.issueTypeRows.length || 1

  return (
    <div className="vf-type-strip">
      <div className="vf-type-strip-head">
        <span className="vf-bold">유형 분포</span>
        <span>{typeDone ? `${rows.length}개 유형` : statusText(statuses[2])}</span>
      </div>
      <div className="vf-type-strip-bars">
        {typeDone ? rows.map(([type, issues]) => {
          const count = asArray(issues).length
          const filter = makeFilter('issue_type', type, typeLabel(type), 'issue_classification')
          return (
            <button
              key={type}
              className={`vf-type-strip-item ${isSameFilter(activeFilter, filter) ? 'vf-type-strip-item--active' : ''}`}
              type="button"
              onClick={() => onSelectFilter(filter)}
            >
              <span>{typeLabel(type)}</span>
              <span className="vf-bold">{count}</span>
              <RatioBar value={count} total={total} />
            </button>
          )
        }) : <div className="vf-overview-empty">유형 분류 대기</div>}
      </div>
    </div>
  )
}

function AuxiliaryStrip({ model, statuses, activeFilter, onSelectFilter }) {
  const slideDone = isStageDone(statuses[4])

  return (
    <div className="vf-aux-strip">
      <TypeDistributionStrip
        model={model}
        statuses={statuses}
        activeFilter={activeFilter}
        onSelectFilter={onSelectFilter}
      />
      <div className="vf-aux-actions">
        <OverviewAction
          label="슬라이드 표기"
          value={slideDone ? model.slideFindings.length : '-'}
          hint={slideDone ? `보고 대상 ${model.slideErrors.summary?.reportable_error_count ?? '-'}개` : ''}
          filter={makeFilter('slide_reportable', 'reportable', '보고 대상 슬라이드 오류', 'slide_review')}
          activeFilter={activeFilter}
          onSelectFilter={onSelectFilter}
          disabled={!slideDone}
        />
...
function MouseTooltip({ children, tooltip, className = '', tooltipClassName = '', tabIndex, ariaLabel, onClick, matchAnchorWidth = false, offset = 6 }) {
...
function StageSummaryCard({ title, status, metrics, children, side = false, chips = [] }) {
  return (
    <section className={`vf-summary-card vf-summary-card--${status} ${side ? 'vf-summary-card--side' : ''}`}>
      <div className="vf-summary-card-head">
        <div className="vf-summary-title-row">
          <span className="vf-bold">{title}</span>
          <HeaderChips chips={chips} />
        </div>
        <span>{statusText(status)}</span>
      </div>
      {asArray(metrics).length ? <MetricStrip items={metrics} /> : null}
      {children}
    </section>
  )
}
...
function SlideSummaryGrid({ model, status }) {
  const isReady = hasDataForTab(model, 'slide_review')
  const summary = asObject(model.slideErrors.summary)
  const slideSource = compactText(model.slideFindings[0]?.source, '-').replace(/_correction$/, '')

  return (
    <section className="vf-slide-summary-grid" aria-label="슬라이드 오류 요약">
      <StageSummaryCard
        title="슬라이드 표기 오류"
        status={status}
        side
        chips={[isReady ? stageModelChip(model, 'slide_review') : '']}
        metrics={[
          ...
          { label: '출처', value: isReady ? slideSource : '-' },
        ]}
      />
    </section>
  )
...
function ModelEvidenceAccordion({ items, valueFormat, title = '모델별 결과' }) {
  if (!asArray(items).length) return null
  return (
...
function ModelDecisionStrip({ models }) {
...
function IssueComparePanel({ model, filterScope, activeFilter }) {
  ...
  const rows = filterRows(model.issueComparisonRows, filterScope, 'claim')
        ...
        {rows.length ? rows.map((item, index) => (
          <article key={item.claim_id || index} className="vf-compare-row">
            <div>
              ...
              <span className="vf-compare-row-status">{agreementLabel(item.agreement?.status)}</span>
            </div>
            <p>{compactText(item.resolved_claim || item.claim_text)}</p>
            <ModelDecisionStrip models={item.models} />
          </article>
...
function ModelBreakdownBoard({ breakdown }) {
  const rows = Object.entries(asObject(breakdown))
  if (!rows.length) return null
  return (
    <div className="vf-classifier-board">
      {rows.map(([modelName, item]) => (
        <div key={modelName} className="vf-classifier-card">
          <div>
            <span className="vf-bold">{compactText(item.resolved_model || modelName)}</span>
            <span className="vf-classifier-card-meta">{compactText(item.provider || item.status, '')}</span>
          </div>
          <CountBars counts={item.breakdown_by_type} total={item.classified_count} />
        </div>
      ))}
    </div>
  )
...
function ContextPreview({ item, resultId }) {
...
function StageSection({ step, title, subtitle, status, metrics, pending, children }) {
  const isDone = status === 'done'
  return (
    <section className={`vf-stage-section vf-stage-section--${status}`}>
      <div className="vf-stage-head">
        <span>{step}</span>
        <div>
          <h2>{title}</h2>
          <p>{subtitle}</p>
        </div>
        <span className="vf-bold">{status === 'done' ? '완료' : status === 'run' ? '진행 중' : '대기'}</span>
      </div>
      <MetricStrip items={metrics} />
      {isDone ? children : <div className="vf-stage-pending">{pending}</div>}
    </section>
  )
}

function ClaimExtractionSection({ model, status, filterScope, activeFilter }) {
  const isDone = status === 'done'
  const claims = isDone ? filterRows(model.claims, filterScope, 'claim') : []
  const contextNeeded = claims.filter(claim => claim.needs_context).length
  const resolved = claims.filter(claim => claim.resolution_status === 'resolved').length
  const approximate = claims.filter(claim => claim.is_approximate).length

  return (
    <StageSection
      step="01"
      title="주장 추출"
      subtitle="발화에서 다음 검증 단계가 다룰 클레임 단위를 만든다."
      status={status}
      pending={pendingText(status, '클레임 추출이 진행 중입니다.', '클레임 추출 대기 중입니다.')}
      metrics={[
        {
          label: 'claims',
          value: isDone ? claims.length : '-',
          subItems: isDone ? [
            { label: 'resolved', value: resolved, divider: true },
            { label: 'needs_context', value: contextNeeded },
            { label: 'approx', value: approximate, divider: true },
          ] : [],
        },
      ]}
    >
      {claims.length ? (
        <div className="vf-record-list">
          ...
          <article key={claim.claim_id || index} className="vf-record">
            <div className="vf-record-head">
              <span className="vf-bold">{compactText(claim.claim_id || `CL${index + 1}`)}</span>
              <ChipList items={[claim.claim_type, claim.resolution_status, claim.is_approximate ? 'approx' : '']} />
              <span className="vf-record-head-meta">{compactText(claim.context_id || asArray(claim.context_ids).join(', '), '')}</span>
            </div>
            <TextBlock label="정규화 클레임">{claim.resolved_claim || claim.claim_text}</TextBlock>
            <TextBlock label="원문 클레임">{claim.claim_text !== claim.resolved_claim ? claim.claim_text : ''}</TextBlock>
            <ChipList items={[claim.claim_fingerprint, asArray(claim.context_ids).join(', ')]} />
          </article>
          ))}
        </div>
      ) : <EmptyFiltered activeFilter={activeFilter} />}
...
function IssueJudgeSection({ model, status, filterScope, activeFilter }) {
  const isDone = status === 'done'
  const issues = isDone ? filterRows(model.issueCandidates, filterScope, 'issue') : []
  const summary = model.issueJudgeSummary
  ...
  return (
    <StageSection
      step="02"
      title="이슈 후보 판단"
      subtitle="추출된 클레임 중 교수 검토 후보로 넘길 항목과 근거를 고른다."
      status={status}
      pending={pendingText(status, '이슈 후보 판단이 진행 중입니다.', '이슈 후보 판단 대기 중입니다.')}
      metrics={[
        { label: '이슈 후보', value: isDone ? issues.length : '-' },
        { label: 'all_models_agreed', value: isDone ? summary.all_models_agreed_count ?? '-' : '-' },
        { label: 'single_model_only', value: isDone ? summary.single_model_only_count ?? '-' : '-' },
        { label: 'no_issue', value: isDone ? summary.no_issue_claim_count ?? '-' : '-' },
      ]}
    >
      ...
      <IssueComparePanel model={model} filterScope={filterScope} activeFilter={activeFilter} />
      {issues.length ? (
        <div className="vf-record-list">
          {issues.map((item, index) => (
          <article key={item.issue_id || index} className="vf-record">
            <div className="vf-record-head">
              <span className="vf-bold">{compactText(item.issue_id || `I${index + 1}`)}</span>
              <ChipList items={[item.claim_id, locationText(item), `신뢰도 ${formatScore(item.confidence)}`, item.representative_model]} />
              <span className="vf-record-head-meta">{asArray(item.detected_by_models).join(', ')}</span>
            </div>
            <TextBlock label="클레임">{item.resolved_claim || item.claim_text}</TextBlock>
            <TextBlock label="후보 이슈">{item.issue}</TextBlock>
            <TextBlock label="후보 선정 근거">{item.candidate_reason}</TextBlock>
            <ModelEvidence items={item.source_model_issues} />
          </article>
          ))}
        </div>
      ) : <EmptyFiltered activeFilter={activeFilter} />}
...
function IssueTypeSection({ model, status, filterScope, activeFilter }) {
  const isDone = status === 'done'
  const rows = isDone ? filterRows(model.issueTypeRows, filterScope, 'issue') : []
  const issuesByType = isDone ? model.issuesByType : {}
  const typeRows = issueTypeCountRows(issuesByType, isDone)

  return (
    <StageSection
      step="03"
      title="이슈 유형 분류"
      subtitle="후보 이슈를 평가 기준별로 묶고, 유형 판단값과 불확실성을 남긴다."
      status={status}
      pending={pendingText(status, '이슈 유형 분류가 진행 중입니다.', '이슈 유형 분류 대기 중입니다.')}
      metrics={[]}
    >
      ...
      {rows.length ? (
        <div className="vf-record-list">
          {rows.map((item, index) => (
          <article key={item.issue_id || item.id || index} className="vf-record">
            <div className="vf-record-head">
              <span className="vf-bold">{compactText(item.issue_id || item.id || `T${index + 1}`)}</span>
              <ChipList items={[
                item.claim_id,
                typeLabel(item.final_issue_type),
                `유형 판단값 ${formatUnitValue(item.ensemble_confidence)}`,
                locationText(item),
              ]} />
            </div>
            <TextBlock label="클레임">{item.resolved_claim || item.claim_text}</TextBlock>
            <ScoreBars scores={item.weighted_scores} valueFormat="unit" />
            <ModelEvidence items={item.model_classifications} valueFormat="unit" />
          </article>
          ))}
        </div>
      ) : <EmptyFiltered activeFilter={activeFilter} />}
...
function FinalVerificationSection({ model, status, resultId, filterScope, activeFilter }) {
  const isDone = status === 'done'
  const rows = isDone ? filterRows(model.severityItems, filterScope, 'issue') : []
  ...
  return (
    <StageSection
      step="04"
      title="최종 평가"
      subtitle="분류된 이슈를 최종 판단하고 교수 검토에 필요한 수정 근거를 정리한다."
      status={status}
      pending={pendingText(status, '최종 평가가 진행 중입니다.', '최종 평가 대기 중입니다.')}
      metrics={[
        { label: '이슈', value: isDone ? summary.total_issue_count ?? rows.length : '-' },
        { label: '고심각', value: isDone ? summary.high_severity_count ?? '-' : '-' },
        { label: '수동 검토', value: isDone ? summary.needs_manual_review_count ?? '-' : '-' },
        { label: '확정/검토/기각', value: isDone ? `${model.counts.confirmed}/${model.counts.review}/${model.counts.rejected}` : '-' },
      ]}
    >
      {rows.length ? (
        <div className="vf-record-list">
          {rows.map((item, index) => {
          const severity = asObject(item.classified_issue_verifier)
          const context = asObject(item.context)
          return (
            <article key={item.issue_id || item.feedback_id || index} className="vf-record">
              <div className="vf-record-head">
                <span className="vf-bold">{compactText(item.issue_id || item.feedback_id || `F${index + 1}`)}</span>
                <ChipList items={[
                  item.claim_id,
                  typeLabel(getIssueType(item)),
                  statusLabel(item.status),
                  `점수 ${formatUnitValue(getScore(item))}`,
                  item.needs_manual_review ? '수동 검토' : '',
                  locationText(item),
                ]} />
              </div>
              <TextBlock label="클레임">{item.resolved_claim || item.claim_text}</TextBlock>
              <ContextPreview item={item} resultId={resultId} />
              <ScoreBars scores={item.previous_classification?.weighted_scores} valueFormat="unit" />
              <ScoreBars scores={{
                valid: severity.average_is_valid_issue,
                severity: severity.average_category_severity,
                context: severity.average_context_resolution,
                disagreement: severity.model_disagreement,
              }} valueFormat="unit" />
              <ChipList items={[
                `컨텍스트: ${context.context_id || asArray(context.context_ids).join(', ')}`,
              ]} />
              <ModelEvidence items={item.model_judgments} valueFormat="unit" />
            </article>
          )
          })}
        </div>
...
function SlideReviewSection({ model, status, resultId, filterScope, activeFilter }) {
  const isDone = status === 'done'
  const rows = isDone ? filterRows(model.slideFindings, filterScope, 'slide') : []
  const summary = asObject(model.slideErrors.summary)
  ...
  return (
    <StageSection
      step="S"
      title="슬라이드 검토 항목"
      subtitle="내용 검증과 별도로 슬라이드 표면 오류를 확인한다."
      status={status}
      pending={pendingText(status, '슬라이드 검토가 진행 중입니다.', '슬라이드 검토 대기 중입니다.')}
      metrics={[
        { label: '슬라이드', value: isDone ? summary.total_slide_count ?? '-' : '-' },
        { label: '오류', value: isDone ? rows.length : '-' },
        { label: '보고 대상', value: isDone ? summary.reportable_error_count ?? '-' : '-' },
        { label: '유형', value: isDone ? Object.keys(summary.breakdown_by_type || {}).length : '-' },
      ]}
    >
      {rows.length ? (
        <div className="vf-record-list">
...
function getClaimFlowSource(row) {
...
function hasIssueInComparison(row) {
...
function issueJudgeFlowMetrics(model, isReady) {
  return [
    { label: 'issue', value: isReady ? model.issueCandidates.length : '-' },
  ]
}
...
function modelCountFrom(...values) {
  const count = values.find(value => asArray(value).length > 0)?.length
  return count || ''
}
...
function isFinalReviewTarget(severity) {
...
function IssueJudgeStatusBadge({ row, severity }) {
...
function jumpToClaimRow(claimId, retries = 2) {
...
function ClaimStageGrid({ row }) {
  const source = getClaimFlowSource(row)
  const type = row.type || row.severity
  const severity = row.severity
  const score = getScore(severity)
  const issueStatus = row.issue || hasIssueInComparison(row)
    ? compactText(row.comparison?.agreement?.status ? agreementLabel(row.comparison.agreement.status) : '이슈 후보')
    : row.comparison ? 'no_issue' : '-'

  return (
    <div className="vf-claim-stage-grid">
      <div>
        <span>클레임</span>
        <span className="vf-bold">{compactText(source.resolution_status || source.claim_type || '추출')}</span>
      </div>
      <div>
        <span>후보 판단</span>
        <span className="vf-bold">{issueStatus}</span>
      </div>
      <div>
        <span>유형</span>
        <span className="vf-bold">{type ? typeLabel(getIssueType(type)) : '-'}</span>
      </div>
      <div>
        <span>최종</span>
        <span className="vf-bold">{severity ? `${statusLabel(severity.status)} · ${formatUnitValue(score)}` : '-'}</span>
      </div>
    </div>
  )
}
...
function getClaimFlowMetrics(model, activeTab, rows) {
  if (activeTab === 'issue_judge') {
    return [
      { label: 'claims', value: rows.length },
      { label: '이슈 후보', value: rows.filter(row => row.issue).length },
      { label: 'no_issue', value: rows.filter(row => row.comparison && !row.issue && !hasIssueInComparison(row)).length },
      { label: 'single_model_only', value: rows.filter(row => row.comparison?.agreement?.status === 'single_model_only').length },
      { label: 'all_models_failed', value: rows.filter(row => row.comparison?.agreement?.status === 'all_models_failed').length },
    ]
  }
  if (activeTab === 'issue_classification') {
    return [
      { label: '분류 완료', value: rows.length },
      { label: '유형', value: new Set(rows.map(row => getIssueType(row.type)).filter(Boolean)).size },
      { label: '원본 이슈', value: model.issueCandidates.length },
    ]
  }
  if (activeTab === 'final_verification') {
    return [
      { label: '이슈', value: rows.length },
      { label: '고심각', value: rows.filter(row => getHighSeverityRows(model).some(item => claimKey(item) === row.claim_id)).length },
      { label: '수동 검토', value: rows.filter(row => row.severity?.needs_manual_review).length },
      { label: '기각', value: rows.filter(row => row.severity?.status === 'rejected').length },
    ]
  }
  return [
    { label: 'claims', value: rows.length },
    { label: '이슈 후보', value: rows.filter(row => row.issue).length },
    { label: '분류 완료', value: rows.filter(row => row.type).length },
    { label: '최종 평가', value: rows.filter(row => row.severity).length },
  ]
}
